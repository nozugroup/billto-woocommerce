 ...
 *
 * The visible failure was "I click Connect and the page just reloads" - the button ended up
 * submitting the settings form, and the code field was gone from the DOM entirely.
 */
...
function connectTab(array $options = []): SettingsTab
{
    ...
    // Klient bez tokenu: `fields()` pyta o listę serii, ale niepodłączony klient nie rusza
    // sieci - a te testy dotyczą wyłącznie znaczników sekcji połączenia.
...
it('keeps form controls out of the section description', function () {
    ...
    // This string goes through wp_kses_post, so markup carrying a form vanishes silently -
    // the dedicated field below is the only place controls survive.
...
it('forgets the installation credentials, not just the tokens, when asked', function () {
    // Production and sandbox are separate BillTo instances with separate registries: a client_id
    // issued in one means nothing in the other, so keeping it would leave a shop that looks
    // connected and gets 401 on every request.
    $p = Options::PREFIX;
    $deleted = [];
