 ...
 *
 * WordPress only updates plugins it can find in the wordpress.org directory. Since WordPress 5.8
 * a plugin can opt out of that lookup with an `Update URI:` header and serve its own updates:
 * WordPress then calls the `update_plugins_{hostname}` filter for that plugin and never asks
 * wordpress.org about the slug. Both halves matter - without the header, a wordpress.org plugin
 * that happens to use the slug `billto-woocommerce` would be offered to our users as an "update".
 *
 * The version is read from BillTo (`/api/v1/plugins/woocommerce/latest`), which relays the latest
 * published GitHub release. The answer is cached in a site transient, so a routine update check
 * costs no HTTP request at all.
 *
 * Updates always come from production billto.pl, even when the shop talks to the sandbox API:
 * the sandbox is for test invoices, not for deciding which plugin version is current.
 */
final class UpdateChecker
{
    ...
    public function check($update, array $pluginData, string $pluginFile)
    {
        ...
        // GitHub source archive would install a directory named after the commit.
