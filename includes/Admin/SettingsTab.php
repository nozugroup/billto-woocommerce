...
final class SettingsTab
{
    ...
    /** Id formularza połączenia wypisywanego w stopce (patrz {@see renderConnectForm()}). */
    ...
    /**
     * Nagłówek sekcji połączenia: stan + przycisk.
     *
     * Przycisk uruchamia przepływ Authorization Code z PKCE, poprzedzony rejestracją tej
     * instalacji (DCR). Sklepikarz wybiera FIRMĘ na ekranie zgody w BillTo, więc wtyczka nigdy
     * nie dostaje dostępu szerszego, niż świadomie nadano - i można go cofnąć po stronie BillTo,
     * bez dotykania sklepu.
     */
    ...
    /**
     * Pole na kod instalacyjny i przycisk połączenia.
     *
     * DLACZEGO WŁASNY TYP POLA, a nie opis sekcji: WooCommerce przepuszcza opisy przez
     * `wp_kses_post`, który wycina `<input>`, `<form>` i atrybut `form`. Formularz umieszczony
     * w opisie po prostu znikał, a przycisk lądował w formularzu ustawień - czyli „Połącz"
     * zapisywało ustawienia i przeładowywało stronę, nie robiąc nic więcej.
     *
     * Kontrolki wskazują atrybutem `form` formularz wypisany w stopce ({@see renderConnectForm()}),
     * bo ta sekcja renderuje się wewnątrz formularza ustawień, a zagnieżdżanie formularzy
     * jest w HTML niedozwolone.
     *
     * POST, nie odsyłacz: kod instalacyjny jest danymi wrażliwymi w skali minut i nie ma prawa
     * wylądować w adresie, historii przeglądarki ani w logu serwera.
     */
    ...
    /**
     * Pusty formularz połączenia, wypisany POZA formularzem ustawień WooCommerce.
     *
     * Kontrolki wskazują go atrybutem `form` - tak HTML5 pozwala mieć osobne wysłanie,
     * bez zagnieżdżania formularzy, czego przeglądarki nie dopuszczają.
     */
    public function renderConnectForm(): void
    {
        ...
        // Wypisywane osobno, a nie wklejane do printf: pole generuje WordPress, a przepuszczanie
        // go przez funkcję escapującą jest i zbędne, i zgłaszane przez phpcs jako nieescapowane.
    ...
    {
        // Odczyt tylko do rozpoznania ekranu - bez zmiany stanu, więc nonce tu nie występuje.
    ...
    public function save(): void
    {
        ...
        // sklep "połączony", który przy każdym żądaniu dostaje 401 - i szukanie przyczyny
    ...
    private function gatewayOptions(): array
    {
        ...
        // WC() bywa null, zanim WooCommerce się zbootuje - bez tego sprawdzenia ustawienia
        // wywracają się fatalem zamiast pokazać pustą listę bramek.
    ...
    private function fields(): array
    {
        ...
        return [
            ...
            // Własny typ pola: kontrolki formularza nie przeżywają `wp_kses_post`, przez który
            // WooCommerce przepuszcza opisy sekcji - patrz renderConnectField().
            ['type' => 'billto_connect', 'id' => $p.'connect'],
            // Pola tokenu API już NIE MA. Wklejony token to długowieczne poświadczenie CAŁEJ
            // firmy, trzymane w bazie sklepu, którego właściciel nie umie zawęzić ani odebrać
            // inaczej niż kasując token używany być może przez coś jeszcze. Po połączeniu OAuth
            // sklep dostaje token jednej firmy, o zakresie widocznym na ekranie zgody.
