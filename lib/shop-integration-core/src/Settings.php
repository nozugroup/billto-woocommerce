...
final class Settings
{
    ...
    /**
     * EU consumer (OSS mode "Polish rates") whose lines carry no VAT in the shop. `block` records
     * an error instead of issuing a zw / 0% invoice; `map` applies the no-tax / zero-rate mappings
     * used on domestic sales, for sellers exempt from VAT.
     *
     ...
     * product rate, as an ancillary supply, instead of the no-tax / zero-rate mapping. Not applied
     * to foreign B2B / non-EU orders, which use their own 0% / np mapping.
     *
     * @var bool
     */
    ...
    /**
     * Foreign buyer (EU company, non-EU) whose products the shop taxed anyway: the order is mapped
     * with Polish rates (`eu_b2b_domestic` / `non_eu_domestic`) rather than 0% WDT / 0% export / np,
     * so the invoice total matches the payment. Set to false to keep the foreign mapping regardless.
     *
     * @var bool
     */
